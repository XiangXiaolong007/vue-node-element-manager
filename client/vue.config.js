...
module.exports = {
    // baseUrl: "/", // 根域上下文目录  从 Vue CLI 3.3 起已弃用，请使用publicPath
    publicPath: "/", // 根域上下文目录
    ...
    configureWebpack: config => { // webpack配置，值为对象时会合并配置，为方法时改写配置
        if(debug) { // 开发环境配置
    ...
    parallel: require('os').cpus().length > 1, // 构建开启多线程处理babel编译
    pluginOptions: {
        // 第三方插件配置
    },
    pwa: {
